...
public class Downloader {
    ...
    public void initDownloadIDE(DownloadHandler downloadHandler) throws IOException {
        ...
        downloadHandler.fileDownloaded(new DownloadEvent(DOWNLOAD_STEP.FETCHING_JSON, "Fetching JSON...", 1, 1));
        ...
        if (isLastedVersionIDE(jsonObject.getAsJsonObject())) {
            ...
            downloadHandler.fileDownloaded(new DownloadEvent(DOWNLOAD_STEP.DOWNLOADING, "Finished", 1, 1));
            return;
        }
        ...
        for (JsonElement jsonElement : assets) {
            ...
            i++;
            downloadHandler.fileDownloaded(new DownloadEvent(DOWNLOAD_STEP.DOWNLOADING, name, i, assets.size()));
        }
    ...
    public void initDownloadFPL(DownloadHandler downloadHandler) throws IOException {
        ...
        if (isLastedVersionFPL(jsonObject.getAsJsonObject())) {
            Logger.log("Lasted version are already installed", this.getClass(), Level.INFO);
            downloadHandler.fileDownloaded(new DownloadEvent(DOWNLOAD_STEP.FINISHED, "Finished", 1, 1));
            return;
        }
        Logger.log("Downloading new version", this.getClass(), Level.INFO);
        String assetsUrl = jsonObject.getAsJsonObject().get("assets_url").getAsString();
    ...
    public static class DownloadEvent {
        ...
        private final DOWNLOAD_STEP download_step;

        public DownloadEvent(DOWNLOAD_STEP download_step, String fileName, int fileNumber, int maxfiles) {
            this.fileName = fileName;
            this.fileNumber = fileNumber;
            this.maxfiles = maxfiles;
            this.download_step = download_step;
        }
        ...
        public DOWNLOAD_STEP getDownload_step() {
            return download_step;
        }
    }

    public enum DOWNLOAD_STEP {
        CHECKING, FETCHING_JSON, DOWNLOADING, FINISHED
    }
}
